...
public class VRWarpper : MonoBehaviour {
	...
	void Start () {
		agent = test.GetComponent<UnityEngine.AI.NavMeshAgent> ();
	...
	void Update () {
		...
		if (rc.Raycast(out hit))
        {
			...
			bool canReach = path.status == UnityEngine.AI.NavMeshPathStatus.PathComplete && cs.Length == 0 && floor; // && nearest && debug1.x == debug2.x && debug1.z == debug2.z;
